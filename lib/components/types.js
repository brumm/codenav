...
const TYPES = {
  ...
  ImportDeclaration: ({ source, specifiers }) => {
    ...
    return (
      <span>
        {[
          ImportDefaultSpecifier && <Spacer>{makeComponentForNode(ImportDefaultSpecifier)}</Spacer>,
          ImportNamespaceSpecifier &&
            <Spacer>{makeComponentForNode(ImportNamespaceSpecifier)}</Spacer>,
          ImportSpecifier &&
            <Spacer>
              <Subtle>{'{ '}</Subtle>
              {ImportSpecifier.map(node => <Spacer>{makeComponentForNode(node)}</Spacer>)}
              <Subtle>{' }'}</Subtle>
            </Spacer>,
        ].filter(Boolean)}
        <Subtle>{` from `}</Subtle>
  ...
  ArrowFunctionExpression: ({ params, body }) => {
    ...
    return (
      <span>
        ...
        {parens && <Subtle>{')'}</Subtle>}
        <Subtle>{' => '}</Subtle>
        {body.type === 'BlockStatement' ? <Subtle>{'{}'}</Subtle> : <Subtle>{'()'}</Subtle>}
      </span>
    )
  },
  ...
  ObjectExpression: () => <Subtle>{'{}'}</Subtle>,
  ...
  VariableDeclaration: ({ declarations }) => declarations.map(({ id }) => makeComponentForNode(id)),
